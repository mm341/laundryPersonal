...
const AuthAndCartListGuard = (props: AuxProps) => {
  ...
  useEffect(
    () => {
      ...
      if (token && cartList?.cart_details?.products?.length>0) {
        setChecked(true);
      } else {
        router.push("/");
      }
...
AuthAndCartListGuard.propTypes = {};

export default AuthAndCartListGuard;
